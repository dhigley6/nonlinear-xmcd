...
def make_figure():
    ...
    axs[0].errorbar(quant['pulse_averaged_absorbed_energies'], quant['spec_energies'], marker='o', label='Expt.', yerr=quant['err_spec_energies'], xerr=0.2*quant['pulse_averaged_absorbed_energies'], linestyle='None', markerfacecolor='k', markeredgecolor='k', color='k', markeredgewidth=1, markersize=3, capsize=2)
...
def format_figure(f, axs):
    ...
    axs[1].text(0.15, 0.15, 'B', fontsize=10, weight='bold', horizontalalignment='left', transform=axs[1].transAxes)
    ...
    axs[0].set_xlabel('Pulse Averaged\nAbsorbed X-Ray\nEnergy (meV/atom)')
    axs[1].set_xlabel('Pulse Averaged\nAbsorbed X-Ray\nEnergy (meV/atom)')
