...
def get_data():
    file_path = os.path.join(os.path.dirname(__file__), '../../data/synchrotron/TrajScan30797.txt')
    data = np.genfromtxt
    data = np.genfromtxt(file_path, skip_header=11)
...
def plot_hysteresis_loop(data):
    ...
    plt.legend(loc='best')
    plt.tight_layout()
    fig_path = os.path.join(os.path.dirname(__file__), 'figure_s1.eps')
    plt.savefig(fig_path, dpi=600)
    fig_path = os.path.join(os.path.dirname(__file__), 'figure_s1.jpg')
    plt.savefig(fig_path, dpi=600)
